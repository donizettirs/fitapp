...
# Title Section
st.markdown("<h2 style='color:yellow;'>Bem vindo ao FITAPP</h2>", unsafe_allow_html=True)
...
# Title Section
st.markdown(f"<div style='font-size:26px;'>🏋️ {day_group}</div>", unsafe_allow_html=True)
st.markdown("Bem vido ao FITAPP selecione ao lado o dia e treino desejado.")
...
if not filtered_df.empty:
    ...
    for index, row in filtered_df.iterrows():
        ...
        with col2:
            ...
            st.markdown(f"**🔢 REPETIÇÕES:** {row['REPETIÇOES']}")
            st.markdown(f"**📅 DIA - GRUPO:** {row['DIA - GRUPO']}")
